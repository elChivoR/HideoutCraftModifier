...
public class UpdateCheckService(ISptLogger<UpdateCheckService> logger)
{
    private const string ReleasesUrl =
        "https://api.github.com/repos/elChivoR/HideoutCraftModifier/releases/latest";
    ...
    private static HttpClient CreateClient()
    {
        ...
        client.DefaultRequestHeaders.UserAgent.ParseAdd("HideoutCraftModifier");
    ...
    private async Task<UpdateInfo?> FetchAsync()
    {
        try
        {
            ...
            // Throws on a non-semver tag (e.g. "v1.0"); caught below and treated as "no info".
            var latest = new SemanticVersioning.Version(latestRaw, loose: true);

            var available = latest > current;
            if (available)
                logger.Info($"[HCM] Update available: {current} -> {latest} ({release.HtmlUrl})");

            return new UpdateInfo(available, latest.ToString(), release.HtmlUrl);
        }
        catch (Exception ex)
        {
            logger.Debug($"[HCM] Update check failed: {ex.Message}");
            return null;
        }
    }
